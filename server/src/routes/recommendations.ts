...
function avoidSore(soreness: Record<string, number>) {
  return (ex: any) => {
    const soreLevel = soreness[ex.primaryMuscle] || 0;
    return soreLevel <= 3; // avoid very sore muscles
  };
}
function hasEquipment(available: string[]) {
  return (ex: any) => ex.equipment.every((e: string) => e === 'bodyweight' || available.includes(e));
}
...
router.post('/', requireAuth, async (req: AuthedRequest, res) => {
  ...
  if (filtered.length === 0) return res.status(400).json({ error: 'No suitable exercises found. Add more equipment or lower soreness.' });
  ...
  try {
    ...
    if (Array.isArray(data?.scores)) {
      const scoreMap = new Map<string, number>(data.scores.map((s: any) => [s.id, s.score]));
    ...
    ranked = filtered.sort((a: any, b: any) => (b.difficulty || 1) - (a.difficulty || 1));
  }
  ...
  while (plan.length < blocks && i < ranked.length) {
    ...
    if (usedMuscles.has(ex.primaryMuscle) && Math.random() < 0.5) continue;
